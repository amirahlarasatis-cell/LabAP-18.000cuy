...
void tambahKendaraan() {
    // Jika parkiran sudah penuh total
    ...
    int slotTersisa = KAPASITAS_MAKS - (int)daftarParkir.size();

    // Jika jumlah yang diminta melebihi kapasitas
    if (jumlah > slotTersisa) {
        cout << "Maaf, parkiran hanya dapat memuat " << KAPASITAS_MAKS << " mobil.\n";
        jumlah = slotTersisa; // tetap proses sampai slot yang tersisa
    }

    for (int i = 0; i < jumlah; i++) {
        Kendaraan* k = new Kendaraan();
        cout << "\nMobil ke " << ((int)daftarParkir.size() + 1) << "\n";
